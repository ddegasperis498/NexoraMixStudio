...
public sealed class AudioTrack : ObservableObject
{
    ...
    private int _phraseLengthBars = 4;
    private int _detectedBeatCount;
    private int _estimatedBarCount;
    ...
    public int PhraseLengthBars { get => _phraseLengthBars; set => SetProperty(ref _phraseLengthBars, Math.Clamp(value, 1, 32)); }
    public int DetectedBeatCount
    {
        get => _detectedBeatCount;
        set
        {
            if (!SetProperty(ref _detectedBeatCount, Math.Max(0, value))) return;
            RaisePropertyChanged(nameof(BeatCountText));
        }
    }

    public int EstimatedBarCount
    {
        get => _estimatedBarCount;
        set
        {
            if (!SetProperty(ref _estimatedBarCount, Math.Max(0, value))) return;
            RaisePropertyChanged(nameof(BarCountText));
        }
    }

    public double AverageBeatIntervalSeconds
    {
        get => _averageBeatIntervalSeconds;
        set => SetProperty(ref _averageBeatIntervalSeconds, Math.Max(0d, value));
    }
    ...
    [JsonIgnore]
    public bool CanOpenExternally => SourceKind == TrackSourceKind.SpotifyReference && !string.IsNullOrWhiteSpace(SpotifyId);

    [JsonIgnore]
    public bool CanAssignToDeck => CanLoadToDeck;
    ...
    [JsonIgnore]
    public string BeatCountText => DetectedBeatCount > 0 ? DetectedBeatCount.ToString() : "—";
    ...
    public string SourceDisplay => CanLoadToDeck
        ...
        : IsExternalOnly
            ? "Solo metadati Spotify · collega un file locale"
            : SourceKind.ToString();
